...
better measures of party influence than Party Unity scores because the
...
on this subset of roll call votes, as well as all votes.
...
cast by a legislator on these party votes.
These scores are highly correlated with Responsiveness, with
...
Two general findings emerged from  Table~\ref{tab-party-unity-regressions}.
...
The relationship between Party Unity scores and Ideological Extremism was an
order of magnitude larger than the relationship between responsiveness and
...
R$^2$ (full model)                & 0.67          & 0.69       & 0.94          & 0.86          \\
R$^2$ (proj model)                & 0.02          & 0.02       & 0.79          & 0.51          \\
Adj. R$^2$ (full model)           & 0.34          & 0.38       & 0.85          & 0.68          \\
...
Based on these analyses, we make two main conclusions.
First, Party Unity scores and Responsiveness to PartyCcalls both capture much of
the same ``signal.''
...
developed in \cite{Minozzi:2013}.
Despite their high correlation, the inferences suggested by models of Party
Unity are implausibly large.
Perhaps more importantly, they lead to a failed placebo test.
This failed test resonates strongly with the modal scholarly interpretation of
