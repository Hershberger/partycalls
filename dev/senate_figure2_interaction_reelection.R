...
fm_extremism <- function(i, j) {
  summary(lm(f_extremism,
    data = subset(senate_data, congress == i & up_for_reelection == j)),
    vcov = vcovHC(type = "HC1"))
}

B <- SE <- data.frame(row.names = 93:112)
B$extremism_elect <- sapply(93:112, function(x)
...
SE$extremism_elect <- sapply(93:112, function(x)
...
pdf(file="plots/senate-figure2-interaction-reelection.pdf", ## RENAME
...
y.ticks <- c(- 24, -12, -6, 0, 12, 24)

b <- B$extremism_elect#[-15]
se <- SE$extremism_elect#[-15]
plot(0, 0, type='n', ylim=c(-6, 28), xlim=c(93, 112),
  cex.lab=1.15, xaxt="n", yaxt="n", xlab="", ylab="Up for Reelection")
...
b <- B$extremism_other#[-15]
se <- SE$extremism_other#[-15]
plot(0, 0, type='n', ylim=c(-6, 28), xlim=c(93, 112),
  cex.lab=1.15, xaxt="n", yaxt="n", xlab="", ylab="Other Congresses in Term")
