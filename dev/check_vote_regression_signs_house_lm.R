...
for (i in 93:112) {
  ...
  regs[[paste0("hou", i)]] <- check_signs(house_party_calls[[paste0("hou", i)]])
}
regs <- rbindlist(regs)

# top code party_t so it plots correctly
regs[party_t > 50, party_t := 50]
regs[party_t < -50, party_t := -50]
...
# get 4x4 table of percent of vote
