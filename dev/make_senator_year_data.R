...
make_senator_time_invariant_covariates <- function() {
  ...
  senator_year_data[, female := 1 * (icpsrLegis %in% female_icpsr)]
  # latino_mc = c("MONTOYA (D NM)", "SUNUNU (R NH)", "SALAZAR (D CO)",
  # "MARTINEZ (R FL)", "MENENDEZ (D NJ)", "RUBIO (R FL)"),
  latino_icpsr = c(6611, 29740, 40500, 40501, 29373, 41102)
  senator_year_data[, latino := 1 * (icpsrLegis %in% latino_icpsr)]

  senator_year_data
}

senator_year_data <- make_senator_time_invariant_covariates()
...
# # NOT CURRENTLY DOING WHAT WE WANT
# # TODO: DECIDE WHETHER TO FIX OR SCRAP
...
setnames(senator_data, "state", "stabb")
...
senator_year_data <- senator_year_data[, .(
  congress, icpsrLegis, mc, stabb, class, caucus, pres_vote_share, vote_share,
