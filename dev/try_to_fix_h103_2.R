...
h103_test1 <- code_party_calls(h103,
  pval_threshold = 0.01, tval_threshold = 2.32, count_min = 15,
  count_max = 150, match_count_min = 150, sim_annealing = FALSE,
  ...
  randomly_reassign_flip_flop_votes_from_noncalls = TRUE)
save(h103_test1,
     file = "test_data/h103_test2.RData")

h103_partycalls <- data.table(
  congress = gsub("[A-Za-z:/\\.]", "", h103_test1$source),
  voteno = h103_test1$party_call_coding$voteno,
  replicate_coding = h103_test1$party_call_coding$coding)

h103_partycalls$gray <-0
h103_partycalls$gray[h103_partycalls$replicate_coding == "gray"] <- 1
mean(h103_partycalls$gray)
