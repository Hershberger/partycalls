...
# build senate year data with both LES data and old methods
...
# get variables from wiseman's dataset
...
senator_year_data[caucus == "Republican", ideological_extremism := -1 * party_free_ideal_point]
...
table(senator_year_data$female.x, senator_year_data$female.y)

# 0 differences in afam
table(senator_year_data$afam.x, senator_year_data$afam.y)

# 5 differences in latino
table(senator_year_data$latino.x, senator_year_data$latino.y)

# 112 differences in leader
table(senator_year_data$leader.x, senator_year_data$leader.y)
