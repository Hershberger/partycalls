...
placebo_pi <- DATA[mean_tr == .5,
  sum((votes < mean(votes)) * y1) - sum((votes > mean(votes)) * y1),
  .(stabb, congress)][,
    mean(V1)]
...
placebo_pf <- DATA[mean_tr == 0,
  sum((votes < mean(votes)) * y2) - sum((votes > mean(votes)) * y2),
  .(stabb, congress)][,
    mean(V1)]
...
seat_type_placebo <- DATA[, sum((votes < mean(votes)) * y1) -
    sum(((votes) > mean(votes)) * y1),
  .(stabb, congress, seat_pair_type)][, mean(V1), .(seat_pair_type)]
...
axis(2, c(-2, -1.5, -1, -.5, 0, 0.5, 1, 1.5, 2), cex.axis = 1.1, labels = TRUE)
