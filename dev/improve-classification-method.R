...
setDT(votedata)
old_partycalls <- votedata[congress == 104,
...
replication_104 <- code_party_calls(
  rc,
  pval_threshold = 0.01,
  count_min = 10,
  count_max = 50,
...
replicate_partycalls <- data.table(
  congress = gsub("[A-Za-z:/\\.]", "", replication_104$source),
  voteno = replication_104$party_call_coding$voteno,
  replicate_coding = replication_104$party_call_coding$coding,
  replicate_pvals = rowMeans(do.call(cbind,
    tail(replication_104$record_of_pvals, 5))))
