...
xtable::xtable(senate_coding_record)
...
get_vote_data <- function(chamber, congress_number) {
  ...
  vote_data <- CJ(vt = colnames(rc$votes), mc = rownames(rc$votes),
    sorted = FALSE)
  ...
  lopside_DT <- vote_data[, .(yea_perc = mean(y, na.rm = TRUE)), .(vt)]
...
sen_lop_coding <- merge(sen_lop_coding, sen_coding,
  by = c("congress", "voteno"))


# get summary stats for lopsided votes
x <- table(sen_lop_coding$lopsided, sen_lop_coding$coding)
xtable::xtable(x)
