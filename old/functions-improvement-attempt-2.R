...
#' @param heat_time
#' @param cool_time
temperature_function <- function(counter, n_votes, heat_time, cool_time,
  curve_mean, curve_sd,
  temperature_method = c("peak", "curve")) {
  stopifnot(temperature_method %in% c("peak", "curve"))
  if (temperature_method == "peak") {
  floor(n_votes * .2 * max(0, 1 - (abs(counter - heat_time) / cool_time)) ^ 2)
  } else {
    if (temperature_method == "curve") {
   floor(n_votes * dnorm(x, mean = curve_peak, sd = curve_sd))
  }
 }
}
