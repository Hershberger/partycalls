...
B <- SE <- data.frame(row.names = 105:112)
B$extremism_gingrich <- sapply(105:112, function(x)
  fm_extremism(x, 1)$coef["ideological_extremism", "Estimate"])
B$extremism_other <- sapply(105:112, function(x)
  fm_extremism(x, 0)$coef["ideological_extremism", "Estimate"])
SE$extremism_gingrich <- sapply(105:112, function(x)
  fm_extremism(x, 1)$coef["ideological_extremism", "Std. Error"])
SE$extremism_other <- sapply(105:112, function(x)
  fm_extremism(x, 0)$coef["ideological_extremism", "Std. Error"])

pdf(file="plots/senate-figure2-gingrich-other-reps.pdf", ## RENAME
...
plot(0, 0, type='n', ylim=c(-16, 22), xlim=c(105, 112),
